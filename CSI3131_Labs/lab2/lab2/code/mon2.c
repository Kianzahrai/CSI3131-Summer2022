...
int main(int argc, char **argv)
{
    ...
    else if (prpid == 0) {  /* Child - start of the program */
        execl(program, program, NULL);
        exit(-1); /* in case where execl fails */
    }
    ...
    else if (fpid == 0) { /* In the child process to run filter */
        if (dup2(pipefd[0], 0) == -1) { /* redirect the standard input to the pipe */
            perror("dup2: ");
            exit(-1);
        }
        close(pipefd[0]); /* no longer require these file descriptors */
        close(pipefd[1]);
    ...
    else if (monpid == 0) {     /* start the procmon but first redirect the standard output to the pipe */
        dup2(pipefd[1], 1);
        close(pipefd[0]);
        close(pipefd[1]);
    ...
    /* No need to communicate witht he pipe */
    close(pipefd[0]);
    close(pipefd[1]);
